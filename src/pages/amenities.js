      ...
      <form onSubmit={(e) => e.preventDefault()} className="mt-8 space-y-5">
        ...
        <div className="mb-4">
          {timeBlocks.map((result) => (
            <TimeBlock id={result.id} setTimeBlocks={setTimeBlocks} timeBlocks={timeBlocks} />
          ))}
        </div>
          ...
          <li key={data.id} className="p-5 bg-white rounded-md drop-shadow-xl">
            <div className="justify-between sm:flex">
              <div className="flex-1">
                <h3 className="text-xl font-medium text-cyan-600">{data.name}</h3>
                <p className="text-gray-500 mt-2 pr-2">{data.description}</p>
              </div>
            </div>
